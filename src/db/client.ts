...
async function initializeNotificationPermissionCompatibility(database: MigrationDatabase) {
  ...
  const needsNotificationPermissionColumn = !columns.some(
    (column) => column.name === 'notification_permission_intro_seen',
  );
  ...
  if (needsNotificationPermissionColumn) {
  ...
  if (needsAnalyticsConsentColumn) {
    await database.execAsync(`
      ALTER TABLE app_settings
