...
test('raise-to-speak enters voice quick add only after the shared Pro gate succeeds', () => {
  ...
  assertSourceIncludes(source, [
    ...
    /blocked:[\s\S]*raiseToSpeakCalibrationPhase === 'success'/,
    /isRaiseToSpeakCalibrating[\s\S]*isRaiseToSpeakSetupBusy[\s\S]*raiseToSpeakCalibrationPhase !== 'awaiting-tilt'/,
    /setIsRaiseToSpeakSetupBusy\(true\);[\s\S]*await updateSettings\(\{[\s\S]*raiseToSpeakEnabled: true/,
