...
- [ ] Google Play内部テストのオプトインURLから最新版をインストールし、アプリを一度起動する
- [ ] 変更前のWidgetをホーム画面から削除してDevelopment Buildを再インストールし、ランチャーのWidget選択画面を開く
- [ ] 「ふわっと。」の選択候補に、ラベンダーテーマのヒーロー予定・キュー予定・件数チップ・削除アイコン・右上の「＋」を含むプレビューが表示される
- [ ] Android 12以降の対応ランチャーでは初期サイズが4×3と表示され、白いグリッド状の仮表示にならない
- [ ] 4×3で追加後、250×180dpのcompact表示として直近2件が収まり、横・縦方向へリサイズできる
- [ ] 表示が更新されない場合は、ランチャーのキャッシュを考慮してアプリをアンインストール後にDevelopment Buildを再インストールして再確認する

#### サイズと表示モード

- [ ] 250×180dp: 先頭1件がヒーロー、次の1件がキューとして表示される
- [ ] 320×220dp: 先頭1件がヒーロー、次の2件がキューとして表示される
- [ ] 360×280dp: 先頭1件がヒーロー、次の3件がキューとして表示される
- [ ] 360×320dp: 先頭1件がヒーロー、次の4件が高さ40dpのキューとして表示される
- [ ] 480×320dp: 直近5件のカード幅が広がり、ヒーロー・日時・余白のバランスが崩れない
- [ ] 360×380dp: 先頭1件とキュー5件が表示される
- [ ] 360×420dp: 先頭1件とキュー6件が表示される
- [ ] 360×460dp以上: 直近8件が表示され、9件目以降はWidget内に表示されない
- [ ] 予定が1件以上ある場合、すべてのサイズで「ふわっと。」と全件数チップが左側のヘッダー、丸い「＋」ボタンが右上に表示される
- [ ] Widget内の表示上限を超える予定がある場合も、右上には表示中ではなく全リマインダー件数が表示される
- [ ] 「＋」はcompactで34dp、それ以外で40dpのヘッダー高に収まり、リサイズ後も右側の余白からずれない

#### テーマ別の素材階層と可読性

- [ ] 設定をskyへ変更すると暖色、lavenderでは紫、mintでは緑のアクセントへWidgetが直ちに更新される
- [ ] 3テーマとも、背景グラデーションと淡い泡は文字より後退し、複雑な壁紙の上でも濃紺文字が読める
- [ ] ヒーローはキューより厚い素材と大きな期限泡を持ち、「次のリマインド」→タイトル→日時→削除の視線順序が明確になる
- [ ] キューは半透明でも各行の境界と40dpの高さが判別でき、背景が文字へ透けすぎない
- [ ] リサイズ後も外周の光の縁、ヒーロー、キューの余白が保たれ、不自然な切れや空白がない

#### 壁紙・データ状態

- [ ] 人物・線・文字が多い複雑な壁紙で、文字や日時の背後へ壁紙が透過して読みにくくならない
- [ ] 明るい単色壁紙と暗い単色壁紙で、テーマ面・テーマ色の追加ボタン・濃紺文字のコントラストが保たれる
- [ ] 予定0件で中央に発光する追加泡、「最初のリマインドを残そう」「タップして追加」が表示され、右上の「＋」は重複表示されず、空状態全体から追加画面を開ける
- [ ] 複数件で、DB取得順の先頭が常にヒーロー、2件目以降が同じ高さのキューとして並ぶ
- [ ] 期限泡がアプリ本体と同じ今日=赤、明日=オレンジ、2〜3日後=黄色、4日以降=水色で表示される
- [ ] 期限泡の半透明グラデーション、枠線、光点が、アプリ本体の締切カラートークンと一致する
- [ ] 各カードに「今日」「明日」「明後日」または「M/d（曜）」と時刻が表示される（明後日より先の日付には曜日を付ける）
- [ ] ヒーローと各キューで通知日時が右寄せの固定幅で表示され、長いタイトルでも日時が押し出されない
- [ ] 見出し・予定タイトル・日時・空状態の案内が丸みのある太字で統一され、硬い印象や文字つぶれがない
- [ ] 表示上限を超えた予定はWidget内に追加表示せず、Widgetの高さに応じて直近2〜8件へ情報量を抑える

#### 操作・更新・表示崩れ

- [ ] 丸い「＋」ボタンから `action=add` の既存導線で追加画面を開ける
- [ ] 各リマインドカードをタップすると既存の `action=view&id=...` 詳細導線でアプリを開ける
- [ ] 各リマインドカード右端にゴミ箱アイコンが表示され、アプリを開かずにタップだけで削除できる
- [ ] ヒーローとキューの詳細導線はカードの高さ全体で反応し、右端のゴミ箱は32×32dpの独立したタップ領域として反応する
- [ ] TalkBackで各カードを選ぶと、タイトル・通知日時・「詳細を開く」が順に読み上げられる
- [ ] 各リマインドカードと丸い「＋」を押した際、押下表示が角丸形状の内側に収まり、四角い枠が表示されない
- [ ] カード内や右端に完了チェック、追加メニューが表示されない
- [ ] 日本語の長いタイトルと英語の長いタイトルが1行で省略・縮小され、領域外へはみ出さない
- [ ] 予定の追加・削除・タイトル更新後に、先頭の昇格とキュー位置が同じWidgetサイズで安定している
- [ ] 設定画面でテーマを変更するとアプリを閉じ直さなくてもWidgetの色が同期される
...
- テーマ別の淡い背景、厚みのあるヒーロー、軽いキュー、期限泡の順に素材階層が分かれ、予定情報が明確に読める
- 先頭予定、残りの予定、全件数、追加操作が競合せず、compactでも最優先の予定を即座に判断できる
