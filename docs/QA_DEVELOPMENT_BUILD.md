...
- [ ] 新規インストールでは「持ち上げて音声入力」がOFFで、初回案内に「使ってみる」「今はしない」が表示される
- [ ] 「今はしない」で権限要求をせず案内が閉じ、設定画面のスイッチから後で有効化できる
- [ ] 「使ってみる」でマイク・モーション権限、近接センサー、日本語の端末内認識を確認し、試し持ち上げが成功した後だけ設定がONになる
- [ ] iOS: マイク、モーションの権限を許可／拒否した各場合にクラッシュせず、拒否時は手入力へ戻れる（端末内認識では音声認識権限を要求しない）
- [ ] Android: 日本語のオフラインモデルがない場合にOSのダウンロード導線が表示され、導入後に再試行できる
- [ ] 近接センサー非搭載端末では自動起動だけが無効になり、入力画面のマイクボタンは利用できる
- [ ] 自然な持ち上げを20回行い、スマホ下部を口元へ近づけたときだけ音声入力が開始する
- [ ] 通常のタップ・スクロール・泡選択・設定操作を各20回行い、誤起動が0回である
- [ ] 端末を下げると音声認識が終了し、文字起こしを編集してから「追加」で保存できる
- [ ] 持ち上げ後、Pro判定やSheet表示中に端末を下げると認識開始がキャンセルされる
...
- [ ] 30秒間持ち上げたままでも安全タイムアウトで終了し、直後1.5秒は再起動しない
- [ ] 録音ファイルが端末に残らず、音声・文字起こし・センサー値がPostHogや外部通信へ送信されない

**受け入れ条件**

- iPhone・Android実機で、持ち上げ20回が自然に起動し、通常操作20回の誤起動が0回である
- 音声は `ja-JP` の端末内認識だけを使用し、端末を下げたときは最終結果を受け取って編集可能な入力状態へ戻る
- バックグラウンド移行・権限拒否・モデル不足・センサー非搭載・認識中断の全経路で手入力を維持する
...
- [ ] 「ふわっと。」の選択候補に、ニュートラルな単色面・予定2件・件数・削除アイコン・「＋」を含むプレビューが表示される
...
- [ ] 250×180dp: 直近2件が縦並びの白い行カードで表示される
- [ ] 320×220dp: 直近3件が余白を保った白い行カードで表示される
- [ ] 360×280dp: 直近4件が縦並びの白い行カードで表示される
...
- [ ] すべてのサイズで「ふわっと。」が左上、全リマインダー件数が右上、丸い「＋」ボタンが右下に表示される
- [ ] Widget内の表示上限を超える予定がある場合も、右上には表示中ではなく全リマインダー件数が表示される
- [ ] すべてのサイズで「＋」のタップ領域が44×44dpあり、リサイズ後も右側の余白からずれない

#### 背景なしの素材階層と可読性

- [ ] すべての時間帯でWidgetの面が同じニュートラル色になり、背景画像・泡・光粒・グラデーションが表示されない
- [ ] 白い行カードがWidget面から穏やかに区別され、タイトル・日時・削除操作の視線順序が明確になる
- [ ] 見出し、予定タイトル、日時、件数の順に文字ウェイトが抑えられ、太字が競合せず小さいサイズでも読める
- [ ] リサイズ後も単色面とカードの余白が保たれ、不自然な切れや空白がない

#### 壁紙・データ状態

- [ ] 人物・線・文字が多い複雑な壁紙で、文字や日時の背後へ壁紙が透過して読みにくくならない
- [ ] 明るい単色壁紙と暗い単色壁紙で、背景面・パールブルーの追加ボタン・濃紺文字のコントラストが保たれる
- [ ] 予定0件でアプリ本体と同じ「まだ泡はひとつも浮いていません」「忘れたくないこと、右下からふわっとどうぞ」が表示され、右下には「＋」ボタンだけが配置される
...
- [ ] 各カードの詳細導線はカードの高さ全体で反応し、右端のゴミ箱は36×36dpの独立したタップ領域として反応する
...
- 単一のニュートラル面、不透明な白い行カード、抑えた文字ウェイトで、予定情報が明確に読める
- パステルの期限ドット、淡いパールブルーの追加ボタン、濃紺文字が選定モックと同じ情報階層で表示される
