...
- Android 12以降でも、通知権限だけで通知設定を確認できる
...
- [ ] Androidで詳細 Sheet のタイトルを編集し、「かきくけこ」へ濁点を付けて「がぎぐげご」と入力できる
- [ ] Androidで詳細 Sheet のタイトルを変換中に、未確定文字や変換候補が消えたり1文字前へ戻ったりしない
- [ ] 詳細 Sheet のタイトル編集を確定すると、表示・ホーム・一覧・検索へ変換後の文字列が反映される
...
- [ ] Android 12以降で現在時刻の1分後を指定し、アプリをバックグラウンドにして、指定時刻から最大60分程度の遅延を許容して通知が届く
- [ ] Android 12以降で現在時刻の1分後を指定し、アプリを終了して、指定時刻から最大60分程度の遅延を許容して通知が届く
- [ ] iOSで現在時刻の1分後を指定し、アプリをバックグラウンドにしても指定時刻に通知が届く

**期待される結果**

- `expo-notifications` の `scheduleNotificationAsync` で予約された通知が、実機で期待されるタイミングに届く
...
- [ ] Android / iOS: 変更前後を同じ予定・画面サイズ・浮遊位置で撮影し、通常時の大きさ、色、透明感、光沢、枠線、文字、配置、タップ領域が一致する
- [ ] Android / iOS: 録画のコマ送りで、0〜55msの微小な揺らぎ、55〜130msの膜の欠け、90〜230msの短い輪郭片、110〜380msの微細な飛沫の順序を確認する
- [ ] Android / iOS: 円全体を縮小して消さず、破裂時に大きなリング・発光・板状や文字の破片が出ない
- [ ] Android / iOS: 標準、最小、横長の泡で画像への切り替わりに明滅・位置ずれ・楕円への変形がない
- [ ] Android / iOS: 表面画像の取得失敗・破断後の取得完了でも待機せず、輪郭片と飛沫だけで完了する（遅い画像へ途中で切り替えない）
- [ ] Android / iOS: 一括選択した泡の破裂が70ms間隔で進み、触覚は先頭だけ。削除失敗時は既存の220msの復元で戻る
...
- [ ] 追加・編集画面の終日スライダーがカプセル形で、白い丸いつまみに「終日」と右矢印が表示される。右端まで動かすと終日になり、時刻選択が隠れる
- [ ] 右端・左端への到達時に軽い振動が1回だけ発生し、端で指を動かし続けても重複しない（iOS / Android の実機）
- [ ] 途中で離す・キャンセルすると元の端へ戻り、終日の状態は変わらず振動しない。勢いの強い短いスワイプでも確定しない
- [ ] 終日から左端へ戻すと時刻選択が再表示され、選択済み時刻が維持される。過去の時刻なら警告が表示され追加できない
- [ ] スライダーが指に追従する。追加画面では入力・日時選択・プレビュー・追加ボタンが同じ領域としてスクロールし、下部だけが独立して残ったり折りたたまれたりしない
- [ ] スライダー内からゆっくり横・斜めにドラッグしても、シートが下がったり閉じたりせず、左右とも端まで操作できる。スライダー内のタップ・縦だけの移動では終日の状態が変わらない
- [ ] スライダー外の内容では縦スクロールでき、シートのハンドルから下へドラッグすると閉じられる
- [ ] キーボード表示中・狭い画面・文字拡大時にも、日付・時刻・通知時刻が読め、つまみと重ならず、追加ボタンを押せる
- [ ] 保存中・音声入力中はスライダーを操作できず、VoiceOver / TalkBackでは状態付きスイッチとして切り替えられる
- [ ] 「動きを減らす」設定では吸着・復帰のアニメーションを省略する。振動を使えない端末でも切り替えられる
- [ ] 今日・明日・任意の日付で、時刻を選ばず終日リマインダーを追加できる
- [ ] 当日の共通通知時刻（初期値9:00）は設定画面から変更できる
- [ ] 通知時刻前の終日は設定した時刻に「今日の『タイトル』をお知らせします」と通知される
- [ ] 終日通知の本文が、新規作成・編集後の再予約・アプリ復帰後の再予約で「今日の『タイトル』をお知らせします」に統一される
- [ ] 通知時刻を過ぎてから今日の終日を追加しても追加直後の通知はなく、当日中は泡・一覧・Widgetに残る
- [ ] 終日の通知後も23:59:59.999までは期限済みにならず、翌日0:00以降に自動消滅ONなら消える
- [ ] 自動消滅OFFでは、翌日以降も終日リマインダーが期限済みとして残る
- [ ] 終日の編集で時刻指定へ戻せ、時刻指定から終日へ切り替えられる。古い通知はキャンセルされる
- [ ] 「今日・終日」「明日・終日」がホーム、一覧、検索、詳細、Android Widgetで同じ表記になる
- [ ] 終日と時刻指定が同じ日なら、終日が先に並ぶ

---

### 12.2 リマインド日時の編集

- [ ] Bubble をタップし、詳細 Sheet の「当日にもう一度お知らせ」カードを押すと日時編集画面が開く
- [ ] 日付と時刻がそれぞれ44dp以上の押しやすい操作行として表示され、現在の日時が下書きとして表示される
- [ ] iOS の確定操作または Android の OK で日時を保存すると、対象日時・前日通知日時・対象日末の有効期限が更新される
- [ ] iOS の閉じる操作または Android のキャンセルでは日時が変更されない
- [ ] 今日以降の日付だけ選択でき、今日の過去時刻を含む過去日時では保存ボタンが無効になる
- [ ] 変更前の当日・前日通知がキャンセルされ、変更後の当日通知と未来の前日通知だけが予約される
- [ ] 前日通知の予定時刻は設定画面の共通時刻を引き継ぎ、日時編集画面に表示される
- [ ] 変更後の前日通知日時がすでに過ぎている場合、「前日のお知らせ時刻は過ぎているため、当日だけ通知します」と表示され、当日通知だけが予約される
- [ ] 前日通知日時が過去のリマインダー詳細では、前日欄が薄く表示され「前日のお知らせ時刻は過ぎています」と案内される
- [ ] 同じ日時を確定してもDB更新・通知のキャンセル・再予約・Widget同期が行われない
- [ ] 通知権限がない状態では、日時は保存されて旧通知はキャンセルされ、「日時は変更しましたが、通知を予約できませんでした」と表示される
- [ ] 上記の状態で権限を許可してアプリを復帰すると、未予約の当日通知が新しい時刻で予約される
- [ ] 保存中は日付・時刻・タイトル・削除を操作できない
- [ ] Android Widget に変更後の日時が反映される

**受け入れ条件**

- 日時編集画面で確定した未来の日時がホーム・一覧・検索・Widgetへ反映され、期限順も更新される
- 予約失敗時も選択した日時を保持し、旧通知を残さず、次回の起動・復帰で未予約通知を修復できる

---

### 12.3 ホームの「次のリマインド」カード

- [ ] 次の予定がある場合、詳細 Sheet と同じ泡背景・薄紫の枠・ベルを使った高さ68dpのカードが表示される
- [ ] カードには「次のリマインド」、タイトル、既存形式の日時が各1行で表示される
- [ ] 360px幅かつ長い日本語タイトルでも、タイトルだけが末尾省略され、曜日付き日時が右端に残る
- [ ] カードをタップすると、そのカードに表示されている予定の詳細 Sheet が開く
- [ ] 押下時に軽い縮小と透明度変化が表示される
- [ ] アクティブな予定が0件の場合はカードが表示されず、Bubble Board の空状態だけが表示される
- [ ] スクリーンリーダーでボタンとして認識され、タイトルと自然な日本語日時が読み上げられる

**受け入れ条件**

- 詳細 Sheet の泡素材・色・角丸・文字階層を継承しつつ、ホームのBubble表示領域を圧迫しない
- カード操作は既存の詳細表示へ接続し、通知・保存データ・並び順を変更しない

---

### 12.4 左右に傾けて音声入力

- [ ] `android.permission.HIGH_SAMPLING_RATE_SENSORS`を含む新しいdevelopment buildを作成して再インストールする（Metroの再読み込みだけで確認しない）
- [ ] 新規インストールでは「左右に傾けて音声入力」がOFFで、初回案内に「使ってみる」「今はしない」が表示される
- [ ] 「今はしない」で権限要求をせず案内が閉じ、設定画面のスイッチから後で有効化できる
...
- [ ] 画面フォーカス、フォアグラウンド復帰、準備処理を待っている間は3秒経過してもセンサー取得失敗を表示しない
- [ ] 加速度センサーの値を3秒間取得できない場合は取得失敗が表示され、「もう一度試す」でセンサー購読を再開できる
- [ ] センサーなし、購読開始失敗、有効値なしで異なる案内が表示され、異常値だけでは傾き進捗や成功状態へ進まない
- [ ] センサー値の取得後10秒間傾きを検出できない場合は、左右へ40度以上回転する案内と「もう一度試す」が表示される
- [ ] キャリブレーション中の端末イラストが実際の左右傾きへ追従し、Reduce Motion時は回転せず段階的なテキストが更新される
- [ ] 縦向きから約40度以上右へ傾ける操作を20回行い、音声入力が自然に開始する
- [ ] 縦向きから約40度以上左へ傾ける操作を20回行い、音声入力が自然に開始する
- [ ] 前後傾け、通常の持ち上げを各20回行い、音声入力が開始しない
- [ ] 通常のタップ・スクロール・泡選択・設定操作を各20回行い、誤起動が0回である
- [ ] 端末を縦向きへ戻すと音声認識が終了し、文字起こしを編集してから「追加」で保存できる
- [ ] 左右どちらかへ傾けた後、Pro判定やSheet表示中に端末を縦へ戻すと認識開始がキャンセルされる
- [ ] 選択モード、詳細Sheet、保存中、Pro判定中、日付・時刻モーダル、バックグラウンドでは自動起動しない
- [ ] Androidでは入力画面が開いている状態でも音声内容が既存タイトルを置換し、iOSでは従来どおり既存タイトルへ空白区切りで追記される
- [ ] Androidで「明日の15時に田中さんに電話」を入力すると、タイトル「田中さんに電話」、日付「明日」、時刻15:00へ反映される
- [ ] Androidで日付だけ／時刻だけを話した場合、話していない方のUI値が維持される
- [ ] Androidで「明日の3時、いや4時に電話」は4:00へ反映され、「明日の10時と15時に薬」は時刻を維持する
- [ ] Androidで「来週」「夜」「週末」など曖昧な表現、9月31日など無効な日時を話しても、該当UI値が維持され、解析警告を表示しない
- [ ] Androidで「30分後に薬」「24時に薬」を入力すると、日跨ぎを含む日付・時刻が反映され、自動登録されず確認後の「追加」で保存できる
- [ ] Androidで昨日・30分前など過去日時を入力しても未来へ補正されず、既存の登録時バリデーションが表示される
- [ ] 明示キャンセルで音声入力開始前のタイトルへ戻り、認識エラーでは取得済み文字が残る
- [ ] 文字起こしが40文字を超えても切り捨てられず、文字数表示と既存の保存制限で編集を促される
- [ ] iOS / Android実機: 音声入力開始イベントと同時に強い振動（Heavy）が1回鳴り、画面を見ずに開始を認識できる。聞き取り表示・アクセシビリティ通知が出て、終了時にも軽いフィードバックがある
- [ ] 開始準備中・マイク権限拒否・開始失敗では、開始を知らせる強い振動が鳴らない
- [ ] Reduce MotionをONにすると音量表示が拡縮せず、初回案内のアニメーションも抑制される
- [ ] Androidでは偽の音量値を生成せず、固定の聞き取り中表示になる
- [ ] 左右どちらかへ傾けたまま8秒を超えても自動終了せず、端末を縦向きへ戻すと認識を確定して直後1.5秒は再起動しない
- [ ] Moonshine固定時に15〜30秒の連続発話・雑音・繰り返し音声を10回入力しても8秒で切れず、`SIGABRT`、`NewStringUTF`、プロセス再起動が発生しない
- [ ] Moonshine固定時に60秒付近の安全上限で録音が終了し、クラッシュ、フリーズ、OOMが発生しない
...
- [ ] 認識終了後にバックグラウンドへ移動し、フォアグラウンド復帰後の再認識でモデルを再読込できる
...
次の文を通常の話速・端末から約30cmの距離で各3回読み上げる。正解文と認識結果はUnicode NFKCへ正規化し、空白と句読点を除いてから文字誤り率（CER）を計算する。文字精度は`1 - CER`、無修正率は認識結果を編集せずそのまま保存できた回数を全試行90回で割って記録する。
...
- [ ] Android / iOS: 60Hz実機で12個表示・単体削除・12個一括削除を各10回確認し、UIフレーム時間（目標16.7ms以内）と入力応答、画像取得時の停止、終了後の画像メモリ解放を記録する
- [ ] Android / iOS: 破裂中も削除対象以外の泡の浮遊が止まらず、残る泡の配置が変わるのは破裂完了後だけ
- [ ] Android / iOS: 画像準備中と破裂中に画面を離れてもクラッシュや遅い完了通知が発生しない
...
- [ ] 設定画面から「<」でホーム画面に戻れる
- [ ] 前日の通知時刻に「すべての泡に共通」と表示される
- [ ] 前日の通知時刻を Picker で確定すると、すべてのアクティブな Bubble の前日時刻が更新される
- [ ] 前日時刻の変更では前日通知だけが置き換わり、当日通知は変更されない
- [ ] 変更前は未来だった前日通知が変更後に過去となる場合、旧通知だけがキャンセルされ、対象件数が案内される
- [ ] 一部の前日通知を予約できなかった場合、反映失敗件数が過去扱い件数と同じ結果メッセージで案内される
- [ ] 前日時刻の反映中は、時刻の再変更操作が無効になる
- [ ] 前日通知の予約に失敗した Bubble は、次回の起動・復帰時に個別に再予約される
- [ ] 旧通知チャンネルを使用していたリマインダーを残した状態でアプリを更新・起動すると、新しい通知チャンネルへ移行される
- [ ] 旧通知チャンネルの移行中にDB保存が失敗した場合、旧通知が失われず、移行Versionが完了扱いにならない
- [ ] 旧通知チャンネルの移行やリマインダー削除で通知キャンセルに失敗した場合、処理が完了扱いにならず、旧通知とリマインダーの整合性が維持される
- [ ] 移行失敗後にアプリを再起動・復帰すると、旧通知を維持したまま移行が再試行される
- [ ] 旧通知チャンネルの移行完了後、同じリマインダーに新旧の重複通知が残らない
- [ ] 当日のデフォルト時刻を Picker で確定すると保存される
- [ ] 終日のお知らせ時刻を Picker で確定すると、未来の終日の当日通知だけが置き換わる
- [ ] 自動消滅の ON/OFF が即座に反映され、再起動後も保持される
- [ ] 自動消滅をOFFにして通知時刻を迎えたBubbleが、アプリ内とAndroid Widgetの両方に残る
- [ ] 自動消滅がOFFでも当日通知が通常どおり届き、通知後もBubbleだけが残る
- [ ] 期限後の通知をタップすると、保持されたBubbleの詳細が開く
- [ ] 未来のBubbleが期限済みBubbleより先に並び、期限済みだけの場合は「期限済み」と表示される
- [ ] 自動消滅をONへ戻すと、OFF中に保持された期限済みBubbleがアプリ内とWidgetから消える
- [ ] 保持された期限済みBubbleを未来へ変更すると、通常のアクティブなBubbleとして通知が再予約される
- [ ] テーマ（sky / lavender / mint）を切り替えると、即座に背景色が変わる

**期待される結果**

- 前日の共通時刻が SQLite に永続化され、既存を含む全アクティブリマインダーの前日通知へ反映される
- 個別の反映失敗で設定変更全体をロールバックせず、未予約の通知を次回の起動・復帰で修復できる
...
Android Widget は Expo Go では確認せず、Widget 対応済みの Development Build をホーム画面へ追加して確認する。先頭の予定を大きな「次のリマインド」、残りをキューとして表示し、アプリで選択中のsky・lavender・mintテーマを軽量なグラデーションと背景泡へ反映する。時間帯別の背景画像やランダム配置は使用しない。AndroidのRemoteViewsは任意の連続アニメーションを実行できず、`updatePeriodMillis` の下限も30分のため、アプリ終了中の期限切れ除外はbest-effortとして扱う。

Google Playの内部テストへ配布したAABでもWidgetを確認できる。テスターがオプトインURLから最新版をPlay Store経由でインストールし、アプリを一度起動した後、ホーム画面を長押ししてWidget一覧から「ふわっと。」を追加する。Play Console内でWidgetを操作する機能ではないため、選択画面・追加・リサイズ・更新・Deep Linkは必ずテスター端末上で確認する。表示が古い場合は、内部テストで対象端末へ配信されているversionCodeを確認し、既存Widgetを削除してアプリを再インストールする。

iOSはWidgetKitのTimelineで予測可能な時刻の表示切り替えと、データ更新時の短い遷移を実装できる。ただし現在のプロジェクトにはWidgetKit Extensionがなく、Expo SDK 54にも公式 `expo-widgets` がないため、このチェックリストではiOSホーム画面Widgetを対象外とする。iOS Widgetを追加する場合は、Extension、App Groupによるデータ共有、Timeline更新を別タスクとして実装・検証する。

#### 2026-09-09 刷新時の検証記録

- Android エミュレーターの Development Build で、本番 Widget コンポーネントをネイティブ描画し、標準・拡大・空状態・長い日本語タイトル・期限済み・3テーマ・最大8件を確認した。
- ネイティブ描画が返す操作領域で、追加／削除が48×48dpであり、詳細領域と重ならないことを確認した。
...
- [ ] 「ふわっと。」の選択候補に、標準サイズの予定1件・表示／取得件数・削除アイコン・右上の「＋」を含むラベンダーのプレビューが表示される
- [ ] Android 12以降の対応ランチャーで初期サイズが4×3になり、横・縦にリサイズできる（セル数と実際のdp寸法の対応はランチャーによる）

#### サイズと表示件数

- [ ] 250×180dp: 予定1件
- [ ] 320×220dp、360×280dp: 予定2件
- [ ] 360×320dp、480×320dp: 予定3件
- [ ] 360×380dp: 予定4件
- [ ] 360×420dp、360×460dp: 予定5件
- [ ] 360×840dp: 最大8件（この大きさを配置できない端末はネイティブプレビューで確認）
- [ ] 各行は64dp、追加・削除のタップ領域は48×48dpを保ち、カードを詰めても文字が読める
- [ ] 取得した予定が表示枠より少ない場合、実際の表示件数を示す
- [ ] ヘッダーは「ふわっと。」の下に「表示中 1件 / 全3件」のように表示する
- [ ] 21件以上を登録しても「全20件」と表示され、取得上限を超えた総件数とは誤認させない

#### テーマと可読性

- [ ] sky（暖色）、lavender（紫）、mint（緑）の設定変更が Widget に反映される
- [ ] 先頭予定は淡い背景で強調され、泡は各行の小さなアクセントとして表示される
- [ ] 複雑な壁紙、明暗の壁紙でタイトルと日時が読める
- [ ] タイトルは14sp以上・1行で末尾を省略し、日本語40文字や長い英語でも縮小しない
- [ ] 日時はタイトルの下に12spで1行表示される
- [ ] 期限済みは先頭・後続行とも日時の前に「期限済み」と表示され、色だけに依存しない
- [ ] 泡の色は既存の締切カラートークンに従い、日時の「今日」「明日」「明後日」・曜日・時刻の書式を維持する
- [ ] 0件では「リマインダーはありません」「＋ 追加する」を表示し、右上の追加ボタンを重複表示しない

#### 操作・同期・アクセシビリティ

- [ ] 右上の「＋」と空状態から既存の `action=add` で追加画面を開ける
- [ ] タイトル・日時の領域から既存の `action=view&id=...` で正しい予定の詳細を開ける
- [ ] 各行右端のゴミ箱からアプリを開かず直接削除できる
- [ ] 削除の48×48dp領域は詳細領域と重ならず、境界付近を押しても両方の操作が発生しない
- [ ] 削除後に該当予定の通知予約がキャンセルされ、アプリと配置済みの複数 Widget に反映される
- [ ] 追加・削除・タイトル更新後に、既存の取得順で先頭予定と後続行が更新される
- [ ] リサイズ前後で予定が重ならず、文字やタップ領域が枠からはみ出さない
- [ ] TalkBackで詳細・削除・追加が別々に選択でき、詳細はタイトル・日時・期限済み状態、削除は対象タイトルと「削除」が読み上げられる
- [ ] 押下表示が角丸領域に収まり、四角い枠が出ない
- [ ] システム文字サイズを拡大しても固定文字サイズの Widget 内で文字が欠けず、TalkBackで省略前のタイトルを確認できる

#### 期限到来時の同期

- [ ] 自動消滅をONにし、現在時刻の1分後を指定したリマインダーを追加する
- [ ] アプリを開いたまま期日を迎えると、画面とWidgetの両方から期日直後にリマインダーが消える
- [ ] 同じ条件のリマインダーを追加後、アプリをホーム画面へ戻して最近のアプリ一覧から終了する（OS設定の「強制停止」は使用しない）
- [ ] アプリを再度開かずに次回のOS定期更新を待つと、期限切れリマインダーがWidgetから消える
- [ ] 省電力モードやDoze中は30分を超えて遅延する可能性があることを記録し、アプリ起動が更新の必須条件になっていないことを確認する
- [ ] 自動消滅をOFFにし、現在時刻の1分後を指定したリマインダーを追加する
- [ ] 通知時刻を迎えた後もアプリとWidgetに残り、通知タップから詳細を開ける
- [ ] アプリを開いたまま通知時刻を迎えても、当日通知がキャンセルされず端末へ届く
- [ ] アプリを終了した状態で通知時刻を迎えても、次回のWidget定期更新後に期限済みリマインダーが残る
- [ ] 自動消滅をONへ戻すと、保持されていた期限済みリマインダーが即座にアプリとWidgetから消える

**期待される結果**

- テーマ別の淡い背景と小さな期限泡を使い、タイトルと日時を明確に読める
- 先頭予定、残りの予定、表示／取得件数、追加操作が競合せず、compactでも最優先の予定を即座に判断できる
- Widget は確認・追加・詳細を開く・簡易削除に役割を限定し、完了操作や追加メニューは表示しない
- アプリ起動中は期日タイマーによるcleanup後にWidgetが同期され、アプリ終了中はAndroidのbest-effortな30分周期更新で期限切れが除外される
