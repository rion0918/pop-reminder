...
- [ ] Android 12以降では、設定画面に「正確な時刻の通知」が表示される
- [ ] Android 12以降で未許可の場合、「正確な通知を許可」からアプリ専用の「アラームとリマインダー」設定を開ける
- [ ] 「アラームとリマインダー」を許可してアプリへ戻ると「許可済み」に更新される

**期待される結果**

- `requestNotificationPermissions()` で権限状態が正しく取得・更新される
- 権限がない場合でもアプリがクラッシュしない
- Android 12以降では通知権限と正確な時刻の通知権限を別々に確認できる
...
- [ ] 通知権限または正確な時刻の通知権限がない状態で追加すると、リマインダーは保存され「通知を予約できませんでした」と表示される
- [ ] 上記の警告から設定画面へ移動し、必要な権限を許可して戻ると未予約のリマインダーが再予約される
- [ ] Android 12以降で現在時刻の1分後を指定し、アプリをバックグラウンドにしても指定時刻に通知が届く
- [ ] Android 12以降で現在時刻の1分後を指定し、アプリを終了しても指定時刻に通知が届く
...
- [ ] Bubble をタップし、詳細 Sheet の鉛筆アイコン付き「当日にもう一度お知らせ」行を押すと時刻 Picker が開く
- [ ] iOS の確定操作または Android の OK で時刻を変更すると、対象日を維持したまま当日の時刻だけが更新される
- [ ] iOS の閉じる操作または Android のキャンセルでは時刻が変更されない
- [ ] 変更後の時刻がホーム・一覧・検索に反映され、並び順も更新される
- [ ] 変更前の当日通知がキャンセルされ、変更後の当日通知だけが予約される
- [ ] 前日の通知日時と通知 ID は変更されない
- [ ] 同じ時刻を確定しても通知のキャンセル・再予約が行われない
- [ ] 当日の過去時刻を選ぶと「過去の時刻には変更できません」と表示され、時刻が保存されない
- [ ] 通知権限または正確な時刻の通知権限がない状態では、時刻は保存されて旧当日通知はキャンセルされ、「時刻は変更しましたが、通知を予約できませんでした」と表示される
- [ ] 上記の状態で権限を許可してアプリを復帰すると、未予約の当日通知が新しい時刻で予約される
- [ ] 保存中は当日時刻・タイトル・削除を操作できない
- [ ] Android Widget に変更後の当日時刻が反映される

**受け入れ条件**

- 日付と前日時刻を変えず、当日の対象時刻と当日通知だけが置き換わる
- 予約失敗時も選択した時刻を保持し、古い時刻の通知を残さず、次回の起動・復帰で修復できる
