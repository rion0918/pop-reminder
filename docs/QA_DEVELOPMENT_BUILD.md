...
- [ ] 30秒間右へ傾けたままでも安全タイムアウトで終了し、直後1.5秒は再起動しない
- [ ] 録音ファイルが端末に残らず、音声・文字起こし・センサー値がPostHogや外部通信へ送信されない

**受け入れ条件**

- iPhone・Android実機で、右傾け20回が自然に起動し、左傾け・通常操作各20回の誤起動が0回である
- 音声は `ja-JP` の端末内認識だけを使用し、端末を縦へ戻したときは最終結果を受け取って編集可能な入力状態へ戻る
