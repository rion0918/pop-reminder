...
- HTTPS で実際に開け、最終更新日が2026年9月7日、運営者がRion（個人開発）、連絡先がrion.developer.apps@gmail.comであることを確認する。
- [実装とデータ申告の確認記録](GOOGLE_PLAY_DATA_REVIEW.md)にある本番設定・SDK通信の確認を完了する。
...
| データ種類                                 | 収集                           | 目的                          | 共有                                       |
| ------------------------------------------ | ------------------------------ | ----------------------------- | ------------------------------------------ |
| Device or other IDs                        | あり（購入用IDの分類も要確認） | Analytics / 購入機能を照合    | 現在の連携設定に第三者共有がない場合はなし |
...
RevenueCatは分析同意とは独立して起動時から動作します。購入用IDや国情報の扱いを含め、Device or other IDs全体を「分析同意後のみ」としないでください。詳細は[確認記録](GOOGLE_PLAY_DATA_REVIEW.md)を参照します。
